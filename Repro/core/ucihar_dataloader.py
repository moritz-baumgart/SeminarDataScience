...
def load_domain_data(domain_idx):
    ...
    """
    data_dir = './data/ucihar_preprocessed/'
...
def prep_domains_ucihar( SLIDING_WINDOW_LEN=0, SLIDING_WINDOW_STEP=0):
    ...
    for source_domain in source_domain_list:
        ...
        source_loader = DataLoader(data_set, batch_size=64, shuffle=False, drop_last=True, sampler=sampler)
    ...
    x = np.transpose(x.reshape((-1, 1, 128, 9)), (0, 2, 1, 3))
    data_set = data_loader_ucihar(x, y, d, transform)
    target_loader = DataLoader(data_set, batch_size=64, shuffle=False)
